...
<script>
    const MODE_ADDITION = 1; // modes for subtraction or addition global constant
    ...
    export default{
        data() {
            return {
                ...
                btnData: [
                    {correct: true, answer: 0}, // array of objects with key value pairs
        ...
        methods: {
            generateQuestion() {
                const firstNumber = this.generateRandomNumber(1, 100); // generate 2 random numbers
                const secondNumber = this.generateRandomNumber(1, 100); // generate 2 random numbers
                const modeNumber = this.generateRandomNumber(1, 2); // generate the mode number
                ...
                switch (modeNumber) {
                    case MODE_ADDITION:
                        ...
                        this.question = `What's ${firstNumber} + ${secondNumber}?`; // string literal
                        break;
        ...
        created() {
            this.generateQuestion(); // question generated on the created lifecycle hook
        }
    }
</script>
